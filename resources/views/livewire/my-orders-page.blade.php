                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">Shipping Method</th>
                        ...
                        <tbody>
                            ...
                            <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800" wire:key="{{$order->id}}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                    <span class="bg-slate-100 dark:bg-slate-700 py-1 px-2 rounded text-xs">
                                        {{ $order->shipping_method ?? 'N/A' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                    @if ($order->tracking_number)
                                        <span class="bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 py-1 px-2 rounded text-xs font-mono">
                                            {{ $order->tracking_number }}
                                        </span>
                                    @else
                                        <span class="text-gray-500 dark:text-gray-400 text-xs">Belum tersedia</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                    <a href="/my-orders/{{$order->id}}" class="bg-slate-600 text-white py-2 px-4 rounded-md hover:bg-slate-500 transition-colors">View Details</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
